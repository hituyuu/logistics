package com.qf.logistics.commons.fdfs;
...
public class FastDFSUtils implements Serializable {
    ...
    static {
        try {
            //clientGloble读配置文件
            ClassPathResource resource = new ClassPathResource("fast-clien.conf");
            ClientGlobal.init(resource.getClassLoader().getResource("fast-clien.conf").getPath());
            System.out.println("di  zhi"+resource.getClassLoader().getResource("fast-clien.conf").getPath());
